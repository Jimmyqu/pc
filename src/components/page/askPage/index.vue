  ...
  <div class="ask-container">
    <div v-if="commitSucc">
      ...
      <div class="ask-main">
          ...
          <el-form ref="form" :model="form">
            <el-form-item>
              ...
              <div class="city-select">
                ...
                <el-select v-model="cityId" clearable placeholder="地级市/州" class="selsct_city" @change="handleCity">
                  <el-option
            ...
            <el-form-item>
              ...
              <div v-if="typeList.length>0">
                ...
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(10,15)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(15,20)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(20,25)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(25,30)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(30,35)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(35,40)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(40,45)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(45,50)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(50,55)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
                <div class="btn-group">
                  <el-button v-for="item,index in  typeList.slice(55,60)" :class="{'dis':item.disable}" :key="index"
                             :disabled="item.disable" @click="handleChoseType(item)">{{item.dept_name}}
                  </el-button>
                </div>
        ...
        <div v-if="setpTwoView">
          <el-form ref="form" :model="setpTwoForm">
            ...
            >
              <p>标题</p>
              <el-input :maxlength="25" v-model="setpTwoForm.title" placeholder="请输入标题"></el-input>
            </el-form-item>
            <el-form-item>
              <p>问题内容</p>
              <el-input
            ...
            <el-form-item style="height: 150px">
               <!-- :file-list='setpTwoForm.fileList' -->
              <el-upload
                :file-list='setpTwoForm.fileList'
      ...
      <div class="large-btn-box">
        ...
        <el-button v-show="setpThreeView" :disabled="submit" @click="handleSubmit">提&nbsp;交</el-button>
    ...
    <div v-else class="succ-container">
          <div class="succ-container-box">
            ...
            <div class="sec">
              <span ><i style="color: #F6574E">{{commitSec}}秒</i>后返回首页</span> <a href="/index.html">问政首页</a>
            </div>
            <p>
              <a href="./reply.html">查看回复率排行</a> | <a href="./satisfied.html">查看满意度排行</a>
            </p>
            <el-button><a href="/index.html" style="color: #fff;text-decoration: none;">立即返回</a></el-button>
          </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        uploadUrl: `http://api.wz.cnhubei.com/api/uploadimg?skey=${this.$cookie.get('token')}&type=pc`,
        btn1: true,
        btn2: true,
        submit: true
      }
    ...
    methods: {
      ...
      handleAvatarSuccess(res, file) {
        let url=res.data.common.app.src
        this.setpTwoForm.url.push({url})
        // console.log(this.setpTwoForm.url)
        // this.setpTwoForm.fileList=
        this.setpTwoForm.fileList.push(
          {
            url:res.data.common.app.src
          }
          )
        // console.log(this.setpTwoForm.fileList)
      ...
      handleAvatarRemove(file) {
        ...
        //
        // this.setpTwoForm.fileList = this.setpTwoForm.fileList.filter(item => item.url !== file.url)
        this.setpTwoForm.url = this.setpTwoForm.url.filter(item => item !== file.url)
        this.setpTwoForm.fileList = this.setpTwoForm.fileList.filter(item => item.url !== file.url)
        console.log(this.setpTwoForm.fileList)
        ...
        if (this.provinceId) {
          try {
            ...
            res.data.data.common.app.map(item => {
              ...
              item.dept_name.length>10?item.dept_name=item.dept_name.slice(0,10)+'..':item.dept_name=item.dept_name
            })
      ...
      handleChoseType(val) {
        ...
        this.firstData = {
          area: `${this.provinceId}|${this.cityId?this.cityId:0}|${ this.areaId?this.areaId:0}`,
    ...
    watch: {
      departmentVal(val) {
        this.firstData.dept_id=''
         if (Array.from(new Set(Object.values(this.firstData))).filter(item=>item).length === 3) {
          this.btn1 = false
        }else{
          this.btn1 = true
        }
